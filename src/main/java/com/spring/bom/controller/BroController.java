	...
	public String login(User_info ui, HttpServletRequest req, String uEmail, Model model) throws Exception {
		...
		if (login == null) {
			...
			if (a == 0) {
				...
				return "redirect:/right/UserdisabledPage";
			} else if (a == 2) {
...
//			s.tempPw(u_id, tempPassword)  ;// db에 비밀번호를 임시비밀번호로 업데이트
	...
	public String uploadForm(HttpServletRequest request, MultipartFile uImage, Model model) throws Exception {
...
//		     File target = new File(requestPath, savedName);
	...
	public String interestAction(Interest in, HttpServletRequest request, Model model,
			@RequestParam(value = "iCode", required = true) List<String> iCode) {
